...
| `dispatch` | For each entry in `response.ids` (NOT `response.tasks` — that's the status snapshot): call `mcp__lazy-dev__dispatch({ run_id, task_id: id })`, then Agent-dispatch using the returned `agent_namespaced` as `subagent_type` and `dispatch_prompt` as the prompt. Loop after each specialist returns. |
| `wait` | Loop immediately — `plan_next` is cheap and idempotent. |
